...
# We can have different slopes
slopes = [[1, 1], [3, 1], [5, 1], [7, 1], [1, 2]]
# Array to store number of trees encountered for different slopes
num_trees_all_slopes = []

for slope in slopes:
  # number of map copies needed horizontally
  num_copies = int((num_rows - 1) * slope[0] / len(map_original[0])) + 1  # if using numpy, use np.ceil instead of +1
  ...
  while row < (num_rows - 1):
    ...
    if map_full[row][column] == "#":
      num_trees += 1
  num_trees_all_slopes.append(num_trees)

print(reduce((lambda x, y: x * y), num_trees_all_slopes))
